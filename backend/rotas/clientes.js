...
router.post('', checkAuth, multer({storage: armazenamento}).single('imagem'), (req, res, next) => {
  ...
  const cliente = new Cliente({
    ...
    criador: req.dadosUsuario.idUsuairo
  })
...
router.delete('/:id', checkAuth, (req, res, next) => {
  Cliente.deleteOne({_id: req.params.id, criador: req.dadosUsuario.idUsuairo}).then(resultado => {
    if (resultado.n > 0){
      res.status(200).json({
...
router.put('/:id', checkAuth, multer({storage: armazenamento}).single('imagem'), (req, res, next) => {
  ...
  Cliente.updateOne({_id: req.params.id, criador:req.dadosUsuario.idUsuairo}, cliente)
  .then((resultado) => {
    // console.log(resultado);
    if(resultado.nModified > 0){
      res.status(200).json({mensagem: 'Atualização realizada com sucesso'});
    }else{
      res.status(401).json({mensagem: 'Atualização não atualizada'});
    }
  }).catch((err) => {
    console.log(err);
  });
